  ...
  traverse(ast, {
    pre: function (node, parent) {
      ...
      if (node.type === 'CallExpression') {
        ...
        if (node.left.type === 'Identifier') {
          ...
          if (obj.type === 'Identifier') {
            current_scope.reference(obj.name, node);
          }
        }
      }
        ...
        var exempt = ['CatchClause', 'FunctionDeclaration', 'VariableDeclarator', 'FunctionExpression', 'AssignmentExpression'];
        if (!in_arr(exempt)(parent.type)) {
          if (parent.type === 'MemberExpression') {
            if (parent.$parent.type !== 'AssignmentExpression' && node !== parent.property) {
              current_scope.reference(node.name, parent);
            }
          }
          else {
      ...
      // places where scope can only be changed by inner scopes
        ...
        var parent_exempt = ['FunctionDeclaration', 'FunctionExpression', 'ForStatement', 'WhileStatement', 'DoWhileStatement', 'SwitchStatement', 'ForInStatement', 'ForOfStatement'];
